...
export const BudgetEnvelopesTab = () => {
  ...
  return (
    <div className="space-y-4" data-testid="envelopes-tab">
          ...
          <div className="flex rounded-lg border-[1.5px] border-m-border-strong overflow-hidden bg-white">
            {[{ id: "programme", label: "Par programme" }, { id: "theme", label: "Par thème stratégique" }].map((a) => (
              <button key={a.id} onClick={() => setAxis(a.id)} data-testid={`envelope-axis-${a.id}`}
                className={`px-3 h-9 text-xs font-bold transition-colors ${axis === a.id ? "bg-m-primary text-white" : "text-m-ink-soft hover:bg-m-lilac"}`}>
                {a.label}
              </button>
            ))}
          </div>
      ...
      {axis === "theme" && canEdit && (
        <div className="flex items-center gap-2">
          <input value={themeName} onChange={(e) => setThemeName(e.target.value)}
            ...
            className="h-9 flex-1 max-w-md bg-white border-[1.5px] border-m-border-strong rounded-lg px-3 text-sm" />
      ...
      {refs.length === 0 ? (
        <div className="bg-white border border-m-border rounded-xl p-8 text-center" data-testid="envelopes-empty">
          <PiggyBank size={32} className="mx-auto text-m-border-strong mb-2" />
          <p className="text-sm text-m-muted">{axis === "programme" ? "Aucun programme." : "Aucun thème stratégique — créez-en un ci-dessus. Rattachez ensuite les projets à un thème depuis leur onglet Informations."}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {refs.map((ref) => {
            ...
            return (
              <div key={ref.id} className="bg-white border border-m-border rounded-xl p-4" data-testid={`envelope-card-${ref.id}`}>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-sm font-bold text-m-ink flex items-center gap-2">
                  ...
                  {axis === "theme" && canEdit && (
                    <button onClick={async () => { await budgetOpsAPI.deleteTheme(ref.id); load(); }}
                      data-testid={`theme-delete-${ref.id}`} className="p-1 text-m-red hover:bg-m-red-soft rounded">
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
                <div className="flex items-center gap-2 mb-3">
                  <input type="number" defaultValue={env?.amount ?? ""} placeholder="Enveloppe (€)"
                    ...
                    className="h-9 flex-1 bg-m-bg border-[1.5px] border-m-border-strong rounded-lg px-3 text-sm font-semibold disabled:opacity-60" />
                  {canEdit && (
                    <button onClick={() => save(ref.id)} disabled={amounts[ref.id] == null} data-testid={`envelope-save-${ref.id}`}
                      className="h-9 px-3 bg-m-blue text-white text-xs font-bold rounded-lg hover:bg-m-blue-dark disabled:opacity-40">
                      OK
                    </button>
                  )}
                </div>
                {env ? (
                  <>
                    <div className="flex justify-between text-[11px] font-semibold mb-1">
                      <span className="text-m-ink-soft">Engagé : {eur(env.engaged)} · Consommé : {eur(env.consumed)}</span>
                      <span style={{ color: rate > 100 ? "#cc4f45" : rate >= 85 ? "#b7791f" : "#3f8a34" }}>{rate} %</span>
                    </div>
                    <div className="h-2 bg-m-lilac rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${Math.min(rate, 100)}%`, backgroundColor: rate > 100 ? "#cc4f45" : rate >= 85 ? "#b7791f" : "#3f8a34" }} />
                    </div>
                  </>
                ) : (
                  <p className="text-[11px] text-m-muted-2">Aucune enveloppe définie pour {year}.</p>
                )}
              </div>
            );
          })}
