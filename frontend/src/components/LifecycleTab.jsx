...
const GATE_BADGES = {
  ...
  pret:          { label: "Prêt pour le comité", cls: "bg-[#e9effe] text-[#2e5fe8] border-[#c9d8fb]" },
  go:            { label: "Go", cls: "bg-[#ddf0d8] text-[#3f8a34] border-[#bfe0b6]" },
  go_reserves:   { label: "Go avec réserves", cls: "bg-[#d5efec] text-[#22766c] border-[#aedcd6]" },
  no_go:         { label: "No-Go", cls: "bg-[#fbe1de] text-[#cc4f45] border-[#f3c1bc]" },
  annule:        { label: "Annulé", cls: "bg-zinc-100 text-zinc-500 border-zinc-200" },
};
const REVIEW_BADGES = {
  pending:         { label: "En attente", cls: "bg-zinc-100 text-zinc-500" },
  valide:          { label: "Validé", cls: "bg-[#ddf0d8] text-[#3f8a34]" },
  valide_reserves: { label: "Validé avec réserves", cls: "bg-[#fdf6e3] text-[#8a6d1a]" },
  refuse:          { label: "Refusé", cls: "bg-[#fbe1de] text-[#cc4f45]" },
};
const VALIDATOR_CHIPS = {
  ARCHITECTE: { label: "Architecture", cls: "bg-[#eceafd] text-[#5b4bc4]" },
  SECURITE:   { label: "Sécurité", cls: "bg-[#fbe1de] text-[#a63c33]" },
  PMO:        { label: "PMO", cls: "bg-[#e9effe] text-[#2e5fe8]" },
};
    ...
    <div className="space-y-5" data-testid="lifecycle-tab">
      ...
      <div className="bg-white border border-[#e8e6f0] rounded-xl shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-heading font-bold text-[#26243a]">Cycle de vie</h3>
          {canDecide && (
            <select
              data-testid="set-phase-select"
              className="text-xs border border-[#e8e6f0] rounded-md px-2 py-1.5 text-zinc-600"
              value={current_phase}
              onChange={async (e) => {
            ...
            <div key={p.key} className="flex items-center gap-1.5">
              <span
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${
                  i < currentIdx ? "bg-[#ddf0d8] text-[#3f8a34] border-[#bfe0b6]"
                  : i === currentIdx ? "bg-[#352c6e] text-white border-[#352c6e]"
                  : "bg-white text-zinc-400 border-[#e8e6f0]"
                }`}
              >
                {p.label}
              </span>
        ...
        {canRequest && !openGate && currentIdx < phases.length - 1 && (
          <button
            ...
            className="mt-4 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-[#2e5fe8] text-white text-xs font-semibold hover:bg-[#2450c8] transition-colors"
          >
      ...
      {/* Gates */}
      {gates.length === 0 && (
        <div className="text-sm text-zinc-400 bg-white border border-[#e8e6f0] rounded-xl p-5" data-testid="lifecycle-no-gates">
      ...
      {gates.map((gate) => {
        ...
        return (
          <div key={gate.gate_id} data-testid={`gate-card-${gate.gate_id}`} className="bg-white border border-[#e8e6f0] rounded-xl shadow-sm p-5 space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              <Diamond size={16} className="text-[#352c6e]" />
              <span className="font-heading font-bold text-[#26243a] text-sm">
              ...
              {isOpen && canRequest && (
                <button data-testid={`gate-cancel-${gate.gate_id}`} onClick={() => cancelGate(gate)} className="text-zinc-300 hover:text-[#cc4f45]" title="Annuler la demande">
                  <Trash2 size={14} />
                </button>
              )}
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#fbfaff] border-b border-[#e8e6f0] text-[10.5px] uppercase tracking-wider font-bold text-[#8a87a0]">
                ...
                {gate.deliverables.map((d) => {
                  ...
                  return (
                    <tr key={d.key} data-testid={`deliverable-row-${d.key}`} className="border-b border-[#f1eff8] align-top">
                      <td className="px-3 py-2.5 font-medium text-[#26243a]">{d.label}</td>
                        ...
                        {isOpen && canRequest ? (
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              className="border border-[#e8e6f0] rounded-md px-2 py-1 text-xs w-44"
                          ...
                          {canReviewThis ? (
                            <div className="space-y-1.5">
                              <input
                                ...
                                className="border border-[#e8e6f0] rounded-md px-2 py-1 text-xs w-40"
                                placeholder="Commentaire (optionnel)"
                                value={comments[rk] || ""}
                                onChange={(e) => setComments((c) => ({ ...c, [rk]: e.target.value }))}
                              />
                              <div className="flex gap-1">
                                <button data-testid={`review-btn-valide-${d.key}`} onClick={() => review(gate, d.key, "valide")} className="px-2 py-1 rounded-md bg-[#ddf0d8] text-[#3f8a34] text-[11px] font-semibold hover:opacity-80" title="Valider"><Check size={12} /></button>
                                ...
                                <button data-testid={`review-btn-refuse-${d.key}`} onClick={() => review(gate, d.key, "refuse")} className="px-2 py-1 rounded-md bg-[#fbe1de] text-[#cc4f45] text-[11px] font-semibold hover:opacity-80" title="Refuser"><X size={12} /></button>
                              </div>
                            </div>
                          ) : (
            ...
            {isOpen && canDecide && (
              <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-[#f1eff8]" data-testid={`gate-decision-bar-${gate.gate_id}`}>
                ...
                <input
                  ...
                  className="border border-[#e8e6f0] rounded-md px-2 py-1.5 text-xs flex-1 min-w-[180px]"
                  placeholder="Commentaire de décision (comité)"
                  value={decisionComment}
                  onChange={(e) => setDecisionComment(e.target.value)}
                />
                <button data-testid="gate-decision-go" onClick={() => decide(gate, "go")} className="px-3 py-1.5 rounded-lg bg-[#3f8a34] text-white text-xs font-semibold hover:opacity-90">Go</button>
                ...
                <button data-testid="gate-decision-nogo" onClick={() => decide(gate, "no_go")} className="px-3 py-1.5 rounded-lg bg-[#cc4f45] text-white text-xs font-semibold hover:opacity-90">No-Go</button>
              </div>
            )}
            {gate.decision && (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()} data-testid="gate-request-modal">
            <h3 className="font-heading font-bold text-[#26243a]">
              Demander le passage {phaseLabel(current_phase)} → {phaseLabel(phases[currentIdx + 1]?.key)}
            </h3>
            <div>
              <label className="text-[11px] uppercase tracking-wider font-bold text-[#8a87a0]">Date de passage souhaitée</label>
              <DateField value={form.target_date} onChange={(v) => setForm((f) => ({ ...f, target_date: v }))} testId="gate-target-date" />
            </div>
            <div>
              <label className="text-[11px] uppercase tracking-wider font-bold text-[#8a87a0]">Instance de gouvernance</label>
              <div className="flex gap-2 mt-1 mb-2">
                <button onClick={() => setForm((f) => ({ ...f, mode: "existing" }))} className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${form.mode === "existing" ? "bg-[#e9effe] text-[#2e5fe8] border-[#c9d8fb]" : "text-zinc-400 border-[#e8e6f0]"}`} data-testid="gate-gov-mode-existing">Comité existant</button>
                <button onClick={() => setForm((f) => ({ ...f, mode: "new" }))} className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${form.mode === "new" ? "bg-[#e9effe] text-[#2e5fe8] border-[#c9d8fb]" : "text-zinc-400 border-[#e8e6f0]"}`} data-testid="gate-gov-mode-new">Nouveau comité</button>
              </div>
              {form.mode === "existing" ? (
                <select
                  data-testid="gate-gov-select"
                  className="w-full border border-[#e8e6f0] rounded-md px-2 py-2 text-sm"
                ...
                <div className="space-y-2">
                  <input data-testid="gate-gov-new-name" className="w-full border border-[#e8e6f0] rounded-md px-2 py-2 text-sm" placeholder="Nom du comité (ex. COPIL Projet X)" value={form.newName} onChange={(e) => setForm((f) => ({ ...f, newName: e.target.value }))} />
                  <div className="flex gap-2">
                    <select data-testid="gate-gov-new-type" className="border border-[#e8e6f0] rounded-md px-2 py-2 text-sm" value={form.newType} onChange={(e) => setForm((f) => ({ ...f, newType: e.target.value }))}>
                      {["copil", "coproj", "comex", "codir", "steering", "autre"].map((t) => <option key={t} value={t}>{t.toUpperCase()}</option>)}
                    </select>
                    <input data-testid="gate-gov-new-date" type="datetime-local" className="flex-1 border border-[#e8e6f0] rounded-md px-2 py-2 text-sm" value={form.newDate} onChange={(e) => setForm((f) => ({ ...f, newDate: e.target.value }))} />
                  </div>
                </div>
              )}
            </div>
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={requestGate} disabled={busy} className="px-3.5 py-2 rounded-lg bg-[#2e5fe8] text-white text-xs font-semibold hover:bg-[#2450c8] disabled:opacity-50" data-testid="gate-modal-submit">
