    ...
    <div data-testid="risk-heatmap">
      ...
      <div className="flex gap-2">
        <div className="flex flex-col items-center justify-center w-3">
          <span className="text-[8px] text-zinc-400" style={{ writingMode: "vertical-rl", textOrientation: "mixed", transform: "rotate(180deg)" }}>
            Probabilité
          </span>
        </div>
        <div className="flex-1">
          <div className="flex flex-col gap-0.5">
            {[5, 4, 3, 2, 1].map((prob) => (
              <div key={prob} className="flex items-center gap-0.5">
                <span className="text-[9px] text-zinc-400 w-3 text-right flex-shrink-0">{prob}</span>
          ...
          <div className="flex gap-0.5 mt-0.5 pl-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex-1 text-center text-[9px] text-zinc-400">{i}</div>
            ))}
          </div>
          <div className="text-[8px] text-zinc-400 text-center mt-0.5">Impact →</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        {[{ label: "Faible (1-6)", cls: "bg-emerald-100 border-emerald-300" },
          ...
          <div key={item.label} className="flex items-center gap-1">
            <span className={`w-3 h-3 rounded-lg border ${item.cls}`} />
      ...
      {hoveredCell && hoveredRisks.length > 0 && (
        <div className="mt-3 border border-zinc-200 rounded-lg bg-zinc-50 p-2.5" data-testid="heatmap-tooltip">
          <div className="text-[10px] font-bold text-zinc-600 mb-1.5">
          ...
          {hoveredRisks.map((r) => {
            ...
            return (
              <div key={r.risk_id} className="text-[11px] text-zinc-700 py-0.5 border-b border-zinc-100 last:border-0">
                <div className="flex items-start gap-1.5">
                  <span className={`font-bold flex-shrink-0 ${cls}`}>{crit}</span>
                  <span className="line-clamp-2">{r.title}</span>
                </div>
                {showProjectName && r.project_name && (
                  <div className="text-[10px] text-zinc-400 pl-4">{r.project_name}</div>
                )}
              </div>
            );
          })}
