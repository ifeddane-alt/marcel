...
function SanityReport({ report }) {
  if (!report || Object.keys(report).length === 0) {
    return <p className="text-xs text-zinc-400 italic">Rapport non disponible</p>;
  }
  const checks = report.checks || [];
  return (
    <div>
      {report.summary && <p className="text-xs text-zinc-600 mb-2">{report.summary}</p>}
      {checks.map((c, i) => (
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8" data-testid="governance-page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-4 border-l-4 border-l-emerald-500">
          <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">Validées</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-4 border-l-4 border-l-rose-500">
          <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">En échec</div>
          <div className="font-heading text-2xl sm:text-3xl font-bold text-rose-600 mt-2">{counts.failed}</div>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-4 border-l-4 border-l-indigo-400">
          <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">Décisions enregistrées</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(filterStatus || filterCategory || filterProject) && (
              <button
                ...
                className="text-xs text-zinc-400 hover:text-zinc-600 px-2 py-1 border border-zinc-200 rounded-lg"
            ...
            <table className="w-full text-sm" data-testid="decisions-global-table">
              ...
              <tbody>
                {filteredDecisions.map((d) => (
                  ...
                  >
                    <td className="px-4 py-2.5 font-mono-data text-xs text-zinc-500 whitespace-nowrap">
                      {d.decision_date ? formatDate(d.decision_date) : "—"}
                    </td>
                    <td className="px-4 py-2.5 max-w-xs">
                      <div className="font-medium text-xs text-zinc-800 line-clamp-2 leading-snug">{d.title}</div>
                    ...
                    <td className="px-4 py-2.5 text-xs text-zinc-500">{d.owner || "—"}</td>
      ...
      <div className={`space-y-3 ${instView === "list" ? "" : "hidden"}`}>
        {sortedInstances.map((g) => {
          ...
          return (
            ...
            >
              <div
                className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-zinc-50/50 transition-colors"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-zinc-800 text-sm truncate" data-testid={`governance-name-${g.governance_id}`}>
                    {g.name}
                  </div>
                ...
                <div className="text-xs text-zinc-500 flex-shrink-0">
                  <span className="font-mono-data font-bold text-zinc-700">{g.projects_scope?.length || 0}</span> projet{(g.projects_scope?.length || 0) > 1 ? "s" : ""}
                </div>
              ...
              {isExpanded && (
                <div className="border-t border-zinc-100 px-5 py-4 bg-zinc-50/50">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        ...
                        {(!g.projects_scope || g.projects_scope.length === 0) && (
                          <span className="text-xs text-zinc-400 italic">Aucun projet défini</span>
                        )}
                      </div>
                    </div>
                    <div>
                  ...
                  <div className="border-t border-zinc-200 pt-4">
                    ...
                    {instanceDecisions.length === 0 ? (
                      <p className="text-xs text-zinc-400 italic">Aucune décision liée à cette instance.</p>
                    ) : (
                      <div className="space-y-2">
                        {instanceDecisions.map((d) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-xs font-medium text-zinc-800 line-clamp-1">{d.title}</div>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className={`text-[10px] font-semibold capitalize ${DECISION_CATEGORY_COLORS[d.category] || "text-zinc-500"}`}>{d.category}</span>
                                {d.owner && <span className="text-[10px] text-zinc-400">· {d.owner}</span>}
                                {d.decision_date && <span className="text-[10px] text-zinc-400">· {formatDate(d.decision_date)}</span>}
                              </div>
                              {d.impact && <div className="text-[10px] text-zinc-400 mt-0.5 line-clamp-1">{d.impact}</div>}
                            </div>
                            {canWrite && (
        ...
        {instances.length === 0 && (
          <div className="bg-white border border-zinc-200 rounded-lg p-10 text-center text-zinc-400 text-sm">
