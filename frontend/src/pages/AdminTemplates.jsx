  ...
  return (
    <div className="border border-zinc-200 rounded-lg overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-2 w-full px-4 py-3 bg-zinc-50 hover:bg-zinc-100 text-left transition-colors"
      >
        <span className="font-semibold text-zinc-700 flex-1">{phase.name || "Phase sans nom"}</span>
        <span className="text-xs text-zinc-400">{phase.milestones?.length || 0} jalons · {phase.tasks?.length || 0} tâches</span>
      ...
      {open && (
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-zinc-500 mb-1 block">Nom de la phase</label>
              <input
                className={INPUT_CLS}
                value={phase.name}
            ...
            <div>
              <label className="text-xs font-semibold text-zinc-500 mb-1 block">Durée par défaut (jours)</label>
              <input
                type="number"
                className={INPUT_CLS}
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-500 mb-1 block">
              Jalons <span className="font-normal text-zinc-400">(un par ligne)</span>
            </label>
            <textarea
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-500 mb-1 block">
              Tâches <span className="font-normal text-zinc-400">(une par ligne)</span>
            </label>
            <textarea
              className={`${INPUT_CLS} resize-none h-16`}
...
function TemplateCard({ tpl, onEdit, onDelete, onDuplicate }) {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden" data-testid={`template-card-${tpl.template_id}`}>
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {tpl.is_default && (
              <span className="text-xs bg-zinc-100 text-zinc-500 px-2 py-0.5 rounded-full">Par défaut</span>
            )}
        ...
        <div className="flex items-center gap-1">
          ...
          {!tpl.is_default && (
            <button onClick={() => onDelete(tpl)} title="Supprimer" className="p-1.5 text-zinc-400 hover:text-rose-600 rounded-lg">
              <Trash2 size={15} />
            </button>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-zinc-100 px-5 py-3 bg-zinc-50 space-y-2">
          {(tpl.phases || []).map(phase => (
            <div key={phase.name} className="flex items-start gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-zinc-400 ml-2">{phase.duration_days_default}j</span>
                {phase.milestones?.length > 0 && (
                  <p className="text-xs text-zinc-500 mt-0.5">Jalons : {phase.milestones.map(m => m.name).join(", ")}</p>
                )}
                {phase.tasks?.length > 0 && (
                  <p className="text-xs text-zinc-400">Tâches : {phase.tasks.map(t => t.name).join(", ")}</p>
                )}
              </div>
            </div>
          ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-lg font-bold text-zinc-800">{template ? "Modifier le template" : "Nouveau template"}</h2>
          <button onClick={onClose}><X size={18} className="text-zinc-400" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-zinc-500 mb-1 block">Nom du template</label>
            ...
            <div>
              <label className="text-xs font-semibold text-zinc-500 mb-1 block">Méthodologie</label>
