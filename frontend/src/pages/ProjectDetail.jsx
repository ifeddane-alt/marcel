  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="font-mono-data text-sm font-bold text-zinc-800">{formatEuro(value)}</span>
      </div>
      <div className="h-2 bg-zinc-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-700 ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <div className="text-[10px] text-zinc-400 mt-0.5 text-right">{pct.toFixed(0)}% du budget total</div>
    </div>
  );
}
...
export default function ProjectDetail() {
  ...
  if (loading) {
    return <div className="p-4 md:p-8 text-zinc-400 text-sm">Chargement du projet...</div>;
  }
  if (!project) {
        ...
        <div className="flex flex-wrap items-center gap-2 flex-shrink-0">
          ...
          {can("export.status_report") && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 border border-emerald-500 text-emerald-700 text-xs sm:text-sm font-semibold rounded-lg hover:bg-emerald-50 transition-colors"
            >
            ...
            <>
              ...
              {canDelete && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 border border-rose-200 rounded-lg text-xs sm:text-sm text-rose-600 hover:bg-rose-50 transition-colors"
                >
      ...
      {(budgetDeviation > project.budget_total * 0.05 || scheduleDelayed) && (
        <div className="mb-4 space-y-2">
          {budgetDeviation > project.budget_total * 0.05 && (
            <div className="flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-lg px-4 py-2.5 text-rose-700 text-sm" data-testid="budget-alert">
              <AlertTriangle size={15} />
              Dépassement budgétaire forecast : +{formatEuro(budgetDeviation)} ({((budgetDeviation / project.budget_total) * 100).toFixed(0)}%)
            </div>
          )}
          {scheduleDelayed && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 text-amber-700 text-sm" data-testid="schedule-alert">
              <Clock size={15} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className={`col-span-1 space-y-4 ${activeTab === "apercu" ? "lg:col-span-8" : "lg:col-span-12"}`}>
          ...
          <div className={`bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-4 md:p-5 ${activeTab === "apercu" ? "" : "hidden"}`} data-testid="budget-section">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-4">
              ...
              <div className="border border-blue-100 rounded-lg p-4 bg-blue-50/30">
                ...
                <div className="space-y-1.5 mb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono-data text-sm font-bold text-zinc-800" data-testid="capex-planned">
              ...
              <div className="border border-amber-100 rounded-lg p-4 bg-amber-50/30">
                ...
                <div className="space-y-1.5 mb-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-zinc-500">Prévu</span>
                    <span className="font-mono-data text-sm font-bold text-zinc-800" data-testid="opex-planned">
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-[10px] text-zinc-500 mb-0.5">Écart vs budget</div>
            ...
            {(project.budget_revision_history || []).length > 0 && (
              <div data-testid="budget-revision-history">
                ...
                <div className="space-y-0 border border-zinc-100 rounded-lg overflow-hidden">
                  {[...project.budget_revision_history].reverse().map((rev, i) => {
                    const isIncrease = rev.new_eac > rev.old_eac;
                    return (
                      <div key={i} className="flex items-start gap-3 px-4 py-2.5 border-b border-zinc-50 last:border-0 hover:bg-zinc-50/50 transition-colors">
                        <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${isIncrease ? "bg-rose-400" : "bg-emerald-400"}`} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-0.5">
                            <span className="font-mono-data text-[11px] text-zinc-400 flex-shrink-0">{rev.date}</span>
                            <span className="flex items-center gap-1.5 text-xs font-mono-data">
                              <span className="text-zinc-500">{Math.round((rev.old_eac || 0) / 1000).toLocaleString("fr-FR")} K€</span>
                            ...
                            {rev.author && (
                              <span className="text-[10px] text-zinc-400 ml-auto flex-shrink-0">{rev.author}</span>
                            )}
                          </div>
                          <div className="text-[11px] text-zinc-500 truncate">{rev.reason}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className={`bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-5 ${activeTab === "apercu" ? "" : "hidden"}`} data-testid="external-costs-section">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-4">
                <div className="border border-orange-100 rounded-lg p-3 bg-orange-50/30">
                  ...
                  <div className="text-[10px] text-zinc-400 mt-0.5">JH alloués × TJM contrat</div>
                </div>
                ...
                <div className="border border-zinc-200 rounded-lg p-3 bg-zinc-50/50">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Total externe</div>
                  <div className="font-mono-data text-xl font-bold text-zinc-800" data-testid="ext-total-cost">
              ...
              <div className="space-y-1.5">
                {externalCosts.resources.map((r) => (
                  <div key={r.resource_id} className="flex items-center justify-between text-xs py-1.5 px-3 bg-zinc-50 rounded-lg border border-zinc-100">
                    <div className="flex items-center gap-2">
                      ...
                      {r.vendor && <span className="text-zinc-400">· {r.vendor}</span>}
                    </div>
                    <div className="font-mono-data text-sm font-bold text-zinc-700">
                      {r.type === "regie"
                ...
                {tasks.length > 0 && (() => {
                  ...
                  return (
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      {/* RAG pill summary */}
                      <div className="flex items-center gap-1.5 bg-zinc-50 border border-zinc-200 rounded-lg px-2.5 py-1" data-testid="tasks-rag-summary">
                        {[
                          ...
                          <span key={key} className="flex items-center gap-1 text-[11px] font-semibold">
                            ...
                            {key !== "red" && <span className="text-zinc-200 mx-1">|</span>}
                          </span>
                        ))}
                      </div>
                      {/* Coverage */}
                      <span className="text-[10px] text-zinc-400">
                        Couverture : <span className="font-semibold text-zinc-600">
                      ...
                      {/* Thresholds reminder */}
                      <span className="text-[9px] text-zinc-300 hidden sm:block">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 font-medium text-zinc-800 max-w-[200px]">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {t.lifecycle_phase && t.lifecycle_phase !== "backlog" && (
                                <span className="text-[9px] font-bold px-1 py-0.5 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-200 flex-shrink-0">
                                  {t.lifecycle_phase.toUpperCase()}
                                </span>
                              )}
                              {t.sprint_id && (() => {
                                return (
                                  <span className="text-[9px] font-bold px-1 py-0.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-200 flex-shrink-0 flex items-center gap-0.5">
                            ...
                            {t.dependencies && t.dependencies.length > 0 && (
                              <div className="flex items-center gap-1 mt-0.5" title={`Prérequis : ${t.dependencies.map(depId => tasks.find(x => x.task_id === depId)?.name || depId).join(", ")}`}>
                                <GitBranch size={9} className="text-zinc-400 flex-shrink-0" />
                          ...
                          <td className="px-3 py-2.5 font-mono-data">
                            <span className={dateAtRisk ? "text-rose-600 font-semibold" : "text-zinc-500"}>
                          ...
                          {/* Budget prévu K€ */}
                          <td className="px-3 py-2.5 text-right font-mono-data text-zinc-500">
                            {t.budget_planned_k > 0 ? `${t.budget_planned_k} K€` : <span className="text-zinc-300">—</span>}
                          </td>
                          ...
                          {/* JH prévus */}
                          <td className="px-3 py-2.5 text-right font-mono-data text-zinc-500">
                            {t.jh_planned > 0 ? `${t.jh_planned} JH` : <span className="text-zinc-300">—</span>}
                          </td>
                          ...
                          {/* Responsable */}
                          <td className="px-3 py-2.5">
                            <span className="text-zinc-600 font-medium">{getResourceName(t.resource_id)}</span>
                          </td>
          ...
          <div className={`bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] ${activeTab === "jalons" ? "" : "hidden"}`} data-testid="milestones-section">
            ...
            {milestones.length === 0 ? (
              <div className="px-5 py-8 text-sm text-zinc-400 text-center">Aucun jalon défini</div>
            ) : (
              <div className="overflow-x-auto">
                    ...
                    {milestones.map((m) => {
                      ...
                      return (
                        <tr key={m.milestone_id} title={tooltipText}
                          className={`border-b border-zinc-100 hover:bg-zinc-50/50 transition-colors ${m.attribute === "critical" ? "border-l-2 border-l-rose-500" : m.attribute === "strategic" ? "border-l-2 border-l-blue-500" : ""}`}
                          ...
                          <td className="px-3 py-2.5">
                            {fCfg ? (
                              <div className="flex flex-col gap-0.5">
                                ...
                                <span className="text-[10px] text-zinc-500">{fCfg.types.find(t=>t.value===m.type)?.label || m.type || "—"}</span>
                              </div>
                            ) : (
                          ...
                          <td className="px-3 py-2.5 text-xs font-mono-data text-zinc-500 whitespace-nowrap">
                            {m.date_actual ? formatDate(m.date_actual) : "—"}
                          </td>
                          <td className="px-3 py-2.5"><MilestoneBadge status={m.status} /></td>
                          <td className="px-3 py-2.5">
                            {m.attribute === "critical" && (
                              <span className="inline-block px-1.5 py-0.5 text-[9px] font-bold uppercase bg-rose-100 text-rose-700 rounded-lg border border-rose-200">Critical</span>
                            )}
                            {m.attribute === "strategic" && (
                              <span className="inline-block px-1.5 py-0.5 text-[9px] font-bold uppercase bg-blue-100 text-blue-700 rounded-lg border border-blue-200">Strategic</span>
                            )}
                          </td>
                          <td className="px-3 py-2.5">
                            {canWrite && (
          ...
          <div className={`bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] ${activeTab === "jalons" ? "" : "hidden"}`} data-testid="dependencies-section">
            ...
            {dependencies.length === 0 ? (
              <div className="px-5 py-8 text-sm text-zinc-400 text-center">Aucune dépendance définie</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    ...
                    {dependencies.map((dep) => {
                      ...
                      return (
                        <tr key={dep.dependency_id}
                          ...
                          <td className="px-3 py-2.5 font-medium text-zinc-800 text-xs">{linkedProject}</td>
                          <td className="px-3 py-2.5 text-xs text-zinc-600">{NATURE_LABELS[dep.nature] || dep.nature}</td>
                          <td className="px-3 py-2.5">
                            <span className={`inline-block px-1.5 py-0.5 text-[9px] font-bold uppercase rounded-lg border ${IMPACT_COLORS[dep.impact] || ""}`}>
                              {dep.impact}
                            </span>
                          </td>
                          <td className="px-3 py-2.5 text-xs font-mono-data text-zinc-600 whitespace-nowrap">
                            {dep.target_date ? formatDate(dep.target_date) : "—"}
                          </td>
                          <td className="px-3 py-2.5 text-xs text-zinc-500">{STATUS_LABELS[dep.status] || dep.status}</td>
                          <td className="px-3 py-2.5">
                            {canWrite && (
                              <div className="flex items-center gap-1">
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5 max-w-xs">
                              <div className="font-medium text-xs text-zinc-800 leading-snug line-clamp-2">{r.title}</div>
                              {r.owner && <div className="text-[10px] text-zinc-400 mt-0.5">{r.owner}</div>}
                            </td>
                            <td className="px-3 py-2.5">
                              <span className={`inline-flex items-center px-1.5 py-0.5 rounded-lg text-[10px] font-semibold capitalize ${catColors[r.category] || "bg-zinc-50 text-zinc-600"}`}>
                                {r.category}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-center font-mono-data text-xs font-bold text-zinc-600">{r.probability}</td>
                            <td className="px-3 py-2.5 text-center font-mono-data text-xs font-bold text-zinc-600">{r.impact}</td>
                            <td className="px-3 py-2.5">
                              <span className={`text-xs font-semibold capitalize ${statusCls[r.status] || "text-zinc-500"}`}>
                                {r.status}
                              </span>
                            </td>
          ...
          {(() => {
            const DECISION_STATUS_COLORS = {
              ...
              reportée:  "bg-zinc-100 text-zinc-600 border-zinc-200",
                    ...
                    <table className="w-full text-sm" data-testid="decisions-table">
                      ...
                      <tbody>
                        {decisions.map((d) => (
                          ...
                          >
                            <td className="px-3 py-2.5 font-mono-data text-xs text-zinc-500 whitespace-nowrap">
                              {d.decision_date ? formatDate(d.decision_date) : "—"}
                            </td>
                            <td className="px-3 py-2.5 max-w-xs">
                              <div className="font-medium text-xs text-zinc-800 leading-snug line-clamp-2">{d.title}</div>
                              {d.impact && <div className="text-[10px] text-zinc-400 mt-0.5 line-clamp-1">{d.impact}</div>}
                            </td>
                            <td className="px-3 py-2.5">
                              <span className={`text-xs font-semibold capitalize ${DECISION_CATEGORY_COLORS[d.category] || "text-zinc-500"}`}>
                                {d.category}
                              </span>
                            </td>
                            <td className="px-3 py-2.5">
                              <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold border ${DECISION_STATUS_COLORS[d.status] || "bg-zinc-100 text-zinc-700"}`}>
                                {d.status}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-xs text-zinc-500">{d.owner || "—"}</td>
                            <td className="px-3 py-2.5 text-xs text-zinc-500 whitespace-nowrap">
                ...
                <tbody>
                  {allocations.map((a) => (
                    <tr key={a.allocation_id} className="border-b border-zinc-100 hover:bg-zinc-50/50" data-testid={`allocation-row-${a.allocation_id}`}>
                      ...
                      <td className="px-4 py-2.5 font-mono-data text-xs text-zinc-700">{a.jh_allocated} JH</td>
                      <td className="px-4 py-2.5 font-mono-data text-xs text-zinc-700">{a.jh_consumed} JH</td>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono-data text-xs text-zinc-600">{a.allocation_rate}%</span>
                        </div>
                      </td>
                    </tr>
                  ))}
                  ...
                  {workAllocations.map((wa) => {
                    ...
                    return (
                      <tr key={wa.work_allocation_id} className="border-b border-zinc-50 hover:bg-zinc-50/50" data-testid={`wa-row-${wa.work_allocation_id}`}>
                        ...
                        <td className="px-3 py-2.5 font-mono text-xs text-zinc-700">{wa.planned_md} JH</td>
                        <td className="px-3 py-2.5 font-mono text-xs text-zinc-700">{wa.consumed_md} JH</td>
                ...
                <tbody>
                  {teamConsumption.map((tc) => (
                    <tr key={tc.team_id || tc.team_name} className="border-b border-zinc-50 hover:bg-zinc-50/50" data-testid={`tc-row-${tc.team_name}`}>
                      ...
                      <td className="px-3 py-2.5 font-mono text-xs text-zinc-600">{tc.planned_md} JH</td>
                      <td className="px-3 py-2.5 font-mono text-xs text-zinc-700 font-semibold">{tc.consumed_md} JH</td>
                      <td className="px-3 py-2.5 font-mono text-xs text-zinc-600">{tc.planned_cost_eur?.toLocaleString("fr-FR")} €</td>
                      <td className="px-3 py-2.5 font-mono text-xs text-zinc-700 font-semibold">{tc.consumed_cost_eur?.toLocaleString("fr-FR")} €</td>
                  ...
                  <div key={snap.snapshot_id} className="px-5 py-4" data-testid={`scope-snap-${snap.snapshot_id}`}>
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold text-zinc-800 text-sm">
                            ...
                            <span className={`ml-2 text-xs px-2 py-0.5 rounded-lg font-medium ${snap.status === "transmitted" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-lg font-semibold">{secCount} SEC</span>
                        <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-lg font-semibold">{etenduCount} ÉTENDU</span>
                        <span className="text-xs bg-zinc-100 text-zinc-500 px-2 py-0.5 rounded-lg">{outCount} OUT</span>
                        {overloadTeams > 0 && (
                          <span className="text-xs bg-red-50 text-red-600 px-2 py-0.5 rounded-lg font-semibold flex items-center gap-1">
                      ...
                      <div className="mt-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                        {snap.capacity_summary.map((team) => (
                          <div key={team.team_id} className={`rounded-lg px-3 py-2 border text-xs
                            ...
                            <div className="font-semibold text-zinc-700 truncate">{team.team_name}</div>
                            ...
                            <div className="text-zinc-400">{team.taux_pct}% · {team.status === "rouge" ? "SURCHARGE" : team.status === "orange" ? "Attention" : "OK"}</div>
                          </div>
                        ))}
                      </div>
                    )}
        ...
        <div className={`col-span-1 lg:col-span-4 space-y-4 ${activeTab === "apercu" ? "" : "hidden"}`}>
          <div className="bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-5">
            ...
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between gap-2 border-b border-zinc-50 pb-2">
                <dt className="text-xs text-zinc-400 font-medium">Statut</dt>
              ...
              {project.end_date_actual && (
                <div className="flex justify-between gap-2 pt-1">
                  <dt className="text-xs text-zinc-400 font-medium">Fin réelle</dt>
          ...
          {raf && (
            <div className="bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-5" data-testid="raf-section">
              ...
              <div className="space-y-2.5">
                {[
                  { label: "Consommé (JH)", value: `${raf.consumed_md} JH`, color: "text-zinc-700" },
          ...
          <div className="bg-white border border-[#e8e6f0] rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-5">
            ...
            {["achieved", "planned", "at_risk", "delayed"].map((s) => {
              ...
              const colors = { achieved: "text-emerald-600", planned: "text-zinc-600", at_risk: "text-amber-600", delayed: "text-rose-600" };
              return (
                <div key={s} className="flex items-center justify-between py-1.5 border-b border-zinc-50 last:border-0">
                  <span className={`text-xs font-medium ${colors[s]}`}>{labels[s]}</span>
                  <span className="font-mono-data text-sm font-bold text-zinc-800">{count}</span>
                </div>
              );
            })}
          </div>
...
const SCOPE_STATUS_CFG = {
  ...
  out:    { label: "OUT",    bg: "bg-zinc-100",  text: "text-zinc-500",   border: "border-zinc-200" },
};
  ...
  return (
    <div className="mt-3 border-t border-zinc-100 pt-3">
      <button onClick={() => setExpanded(!expanded)}
        className="flex items-center gap-1 text-xs font-semibold text-zinc-500 hover:text-zinc-800 mb-2">
        ...
        {visible.map(f => {
          ...
          return (
            <button key={f.task_id || f.id} onClick={() => setSelected(f)}
              ...
              <span className="text-xs text-zinc-700 font-medium truncate flex-1">{f.name || f.title}</span>
              {(f.total_jh_estimated || 0) > 0 && <span className="text-[10px] text-zinc-400">{f.total_jh_estimated} JH</span>}
...
function ScopeFeatureModal({ feature: f, onClose }) {
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-0 sm:p-4" data-testid="scope-feature-modal">
      <div className="bg-white rounded-none sm:rounded-2xl shadow-2xl w-full max-h-screen sm:max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="px-5 py-4 space-y-3 text-sm">
          {f.description && (
            <div>
              <div className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-1">Description</div>
              <p className="text-zinc-700 text-xs">{f.description}</p>
            </div>
          )}
          {f.pmo_comment && (
            <div>
              <div className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-1">Commentaire PMO</div>
              <p className="text-zinc-600 text-xs bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 italic">{f.pmo_comment}</p>
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            {(f.total_jh_estimated || 0) > 0 && (
              <div className="bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2">
                <div className="text-[10px] text-zinc-400 font-semibold uppercase">JH estimés</div>
                <div className="text-lg font-bold text-zinc-800">{f.total_jh_estimated}</div>
              </div>
            )}
            {f.team_owner && (
              <div className="bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2">
                <div className="text-[10px] text-zinc-400 font-semibold uppercase">Équipe owner</div>
                <div className="text-sm font-semibold text-zinc-700">{f.team_owner}</div>
              </div>
            )}
          </div>
          {f.jh_by_phase && Object.keys(f.jh_by_phase).length > 0 && (
            <div>
              <div className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">Estimations par phase</div>
