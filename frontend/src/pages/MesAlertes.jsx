...
function AlertRuleModal({ onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-0 sm:p-4" data-testid="alert-rule-modal">
      <div className="bg-white rounded-none sm:rounded-2xl shadow-2xl w-full max-h-screen sm:max-h-[90vh] overflow-y-auto sm:max-w-md">
        ...
        <div className="px-5 py-4 space-y-4">
          {/* Métrique */}
          <div>
            <label className="block text-xs font-semibold text-zinc-700 mb-1.5">Indicateur surveillé</label>
            <select
              value={metric}
              onChange={e => setMetric(e.target.value)}
              className="w-full text-sm border border-zinc-200 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400"
          ...
          <div>
            <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
              Seuil d'alerte {metaCfg && <span className="text-zinc-400 font-normal">({metaCfg.unit})</span>}
            </label>
            <input
              type="number"
              value={threshold}
          ...
          {/* Périmètre */}
          <div>
            <label className="block text-xs font-semibold text-zinc-700 mb-1.5">Périmètre</label>
            <select
              value={scope}
          ...
          <div>
            <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
              Libellé personnalisé <span className="text-zinc-400 font-normal">(optionnel)</span>
            </label>
            <input
              type="text"
              value={label}
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-zinc-100">
          <button onClick={onClose} className="text-sm text-zinc-500 hover:text-zinc-700 px-4 py-2 rounded-lg border border-zinc-200">
            Annuler
          </button>
          <button
            onClick={handleSave}
      ...
      <div className="bg-white border border-zinc-200 rounded-lg shadow-sm" data-testid="alert-rules-table">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-zinc-600 font-semibold text-sm">Aucune règle configurée</p>
            <p className="text-zinc-400 text-xs mt-1">Cliquez sur "Nouvelle règle" pour commencer.</p>
          </div>
        ) : (
          <table className="w-full text-sm" data-testid="alert-rules-list">
            ...
            <tbody>
              {rules.map(rule => {
                ...
                return (
                  <tr key={rule.rule_id} className="border-b border-zinc-100 hover:bg-zinc-50/60 transition-colors" data-testid={`alert-rule-row-${rule.rule_id}`}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Icon size={13} className="text-zinc-500" />
                        <span className="text-xs font-medium text-zinc-700">{metaCfg?.label || rule.metric}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 font-mono-data text-sm font-bold text-zinc-800">
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {rule.enabled
                          ...
                          : <><ToggleLeft size={18} className="text-zinc-300" /><span className="text-zinc-400">Inactif</span></>
                        }
                      </button>
                    </td>
                    <td className="px-4 py-3">
