...
export default function Import() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-4xl" data-testid="import-page">
        ...
        {STEP_LABELS.map((label, i) => (
          <React.Fragment key={i}>
            ...
            {i < STEP_LABELS.length - 1 && (
              <div className={`flex-1 h-px mx-3 ${i < step ? "bg-emerald-400" : "bg-zinc-200"}`} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      {error && (
        <div className="mb-4 flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-lg px-4 py-3 text-rose-700 text-sm" data-testid="import-error">
        ...
        <div className="space-y-5" data-testid="import-step-mapping">
          <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] text-zinc-400">{preview.columns.length} colonnes CSV</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {preview.columns.map((col) => (
                <div key={col} className="flex items-center gap-3 bg-zinc-50 rounded-lg px-3 py-2.5 border border-zinc-100">
                  <span className="text-xs font-mono text-zinc-600 flex-1 truncate" title={col}>{col}</span>
            ...
            <p className="text-[11px] text-zinc-400 mt-3">* Champs obligatoires</p>
          </div>
              ...
              <table className="w-full text-xs" data-testid="preview-table">
                <thead>
                  <tr className="bg-zinc-50 border-b border-zinc-100">
                    {preview.columns.map((col) => (
                      <th key={col} className="px-3 py-2 text-left font-semibold text-zinc-600 whitespace-nowrap">{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {preview.preview_rows.map(({ row_num, data }) => (
                    <tr key={row_num} className="border-b border-zinc-50 hover:bg-zinc-50/50">
                      {preview.columns.map((col) => (
                        <td key={col} className="px-3 py-2 text-zinc-600 whitespace-nowrap max-w-[150px] truncate" title={data[col]}>
                          {data[col] || <span className="text-zinc-300">—</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => setStep(0)} className="flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-700">
        ...
        <div className="space-y-5" data-testid="import-step-validation">
            ...
            {validationResult.errors.length === 0 ? (
              ...
              <div className="space-y-2" data-testid="validation-errors">
                {validationResult.errors.map((e, i) => (
                  <div key={i} className="flex items-start gap-3 bg-rose-50 border border-rose-100 rounded-lg px-3 py-2.5">
          ...
          <div className="bg-zinc-50 border border-zinc-200 rounded-lg p-4 flex items-center gap-3">
            <div className="text-sm text-zinc-600 flex-1">
              <span className="font-semibold text-zinc-950">{preview?.total_rows} lignes</span> seront traitées.
              Les lignes en erreur seront ignorées, les lignes valides seront importées.
            </div>
          </div>

          <div className="flex items-center justify-between">
            <button onClick={() => setStep(1)} className="flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-700">
              <ArrowLeft size={14} /> Retour
            </button>
            <button
        ...
        <div className="space-y-5" data-testid="import-step-result">
          <div className={`border rounded-lg p-6 ${commitResult.skipped === 0 ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Total traité", value: commitResult.total_rows, color: "text-zinc-800" },
                { label: "Créés", value: commitResult.created, color: "text-emerald-700" },
                { label: "Ignorés", value: commitResult.skipped, color: commitResult.skipped > 0 ? "text-rose-600" : "text-zinc-400" },
              ].map(({ label, value, color }) => (
                <div key={label} className="bg-white border border-zinc-200 rounded-lg p-4 text-center">
            ...
            <div className="bg-white border border-zinc-200 rounded-lg shadow-sm">
              ...
              <div className="p-4 space-y-2 max-h-64 overflow-y-auto" data-testid="commit-errors">
                {commitResult.errors.map((e, i) => (
                  <div key={i} className="flex items-start gap-3 bg-rose-50 border border-rose-100 rounded-lg px-3 py-2">
                    <AlertTriangle size={13} className="text-rose-500 mt-0.5 flex-shrink-0" />
                    <div className="text-xs">
                      <span className="font-semibold text-rose-700">Ligne {e.row}</span>
                      <span className="text-zinc-400 mx-1">·</span>
                      <span className="font-mono text-zinc-600">{e.field}</span>
                      <span className="text-zinc-500 ml-1">: {e.message}</span>
                    </div>
                  </div>
                ))}
          ...
          <div className="flex justify-between">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-zinc-700 text-white text-sm font-semibold rounded-lg hover:bg-zinc-800 transition-colors"
            >
