...
function Kpi({ label, value, sub, donut }) {
  return (
    <div className="bg-white border border-m-border rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] p-4 flex items-center gap-3.5">
      {donut}
  ...
  if (!resource) {
    return (
      <div className="p-8">
        ...
        <Link to="/resources" className="text-sm text-m-blue font-semibold hover:underline mt-2 inline-block">← Retour aux ressources</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8" data-testid="resource-detail-page">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-m-muted mb-4">
        <span>Accueil</span>
        <span>/</span>
        <Link to="/resources" className="hover:text-m-blue">
          Ressources
        </Link>
        <span>/</span>
        <span className="text-m-primary font-semibold">{resource.name}</span>
      </nav>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        <div className="min-w-0">
          <h1 className="font-heading text-2xl sm:text-[28px] font-extrabold text-m-ink leading-tight flex flex-wrap items-center gap-x-3 gap-y-1" data-testid="resource-detail-name">
            {resource.name}
          ...
          <div className="text-sm text-m-ink-soft mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {resource.team && <span className="text-xs bg-m-lilac text-m-primary-dark px-2 py-0.5 rounded-lg border border-m-border-lav">{resource.team}</span>}
            {resource.tjm_eur != null && <span className="font-mono-data text-xs">TJM {resource.tjm_eur.toLocaleString("fr-FR")} €</span>}
            <span className="text-xs text-m-muted">Disponibilité {availRate}%</span>
            {resource.entry_date && (
              <span className="text-xs text-m-muted" data-testid="resource-entry-date">
                Entrée le <b className="font-mono-data text-m-ink-soft">{new Date(resource.entry_date).toLocaleDateString("fr-FR")}</b>
              </span>
            )}
            {resource.contract_ref && (
              <span className="font-mono-data text-[10.5px] font-semibold text-m-primary-dark bg-m-lilac border border-m-border-lav px-1.5 py-px rounded" data-testid="resource-contract-ref">
                {resource.contract_ref}
              </span>
            )}
            {resource.contract_end && (() => {
              ...
              return days < 0 ? (
                ...
                <span className="text-xs text-m-muted" data-testid="resource-contract-expiry">Contrat jusqu'au <b className="font-mono-data text-m-ink-soft">{d}</b></span>
              );
            })()}
          </div>
        </div>
      ...
      {(() => {
        ...
        return (
          <div className="bg-white border border-m-border rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] mb-6 overflow-hidden" data-testid="resource-load-timeline">
            <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 border-b border-m-border-soft">
              <span className="font-heading text-[13px] font-bold text-m-ink">Charge mensuelle</span>
              <div className="flex items-center gap-3 text-[10.5px] font-semibold text-m-muted">
                ...
                <span className="text-m-muted-2">Capacité effective : {capaEffective} JH/mois</span>
              </div>
            </div>
            {monthly.length === 0 ? (
              ...
              <div className="flex items-end gap-4 px-5 pt-4 pb-3 overflow-x-auto">
                {monthly.map((m) => (
                  <div key={m.month} className="flex flex-col items-center gap-1 min-w-[54px]" data-testid={`load-month-${m.month}`}>
                    ...
                    <div className="relative h-24 w-9 bg-m-lilac rounded-md overflow-hidden flex items-end">
                      <div
                        className="absolute left-0 right-0 border-t border-dashed border-m-muted-2 z-10"
                    ...
                    <span className="font-mono-data text-[10px] font-semibold text-m-ink">{m.jh} JH</span>
                    <span className="text-[9.5px] text-m-muted capitalize whitespace-nowrap">{fmtMonth(m.month)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="bg-white border border-m-border rounded-xl shadow-[0_2px_8px_-3px_rgba(53,44,110,0.08)] overflow-hidden" data-testid="resource-allocations-section">
        <div className="flex items-center justify-between px-5 py-3 border-b border-m-border-soft">
          <span className="font-heading text-[13px] font-bold text-m-ink">Allocations par projet ({byProject.length})</span>
        </div>
        {byProject.length === 0 ? (
            ...
            <thead>
              <tr className="bg-m-bg border-b border-m-border text-left">
                {["", "Projet", "Période", "Mois", "JH alloués", "JH consommés", "Consommation"].map((h, i) => (
                  <th key={i} className="px-4 py-2.5 text-[10.5px] uppercase tracking-wider font-bold text-m-muted">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {byProject.map((g) => {
                ...
                return (
                  <React.Fragment key={g.projectId}>
                    <tr
                      className="border-b border-m-border-soft hover:bg-m-bg transition-colors cursor-pointer"
                      onClick={() => toggle(g.projectId)}
                      data-testid={`resource-alloc-project-${g.projectId}`}
                    >
                      <td className="pl-4 py-3 w-6 text-m-muted">
                        {isOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2 min-w-0">
                          {g.project?.code && (
                            <span className="font-mono-data text-[10.5px] font-semibold text-m-primary-dark bg-m-lilac border border-m-border-lav px-1.5 py-px rounded flex-shrink-0">
                              {g.project.code}
                            </span>
                          )}
                          {g.project ? (
                            <Link
                              ...
                              className="font-semibold text-m-ink hover:text-m-blue hover:underline truncate"
                      ...
                      <td className="px-4 py-3 text-xs text-m-ink-soft whitespace-nowrap">
                        {fmtMonth(g.from)} → {fmtMonth(g.to)}
                      </td>
                      <td className="px-4 py-3 font-mono-data text-xs text-m-ink-soft">{g.rows.length}</td>
                      <td className="px-4 py-3 font-mono-data text-sm font-bold text-m-ink">{g.allocated} JH</td>
                      <td className="px-4 py-3 font-mono-data text-sm text-m-ink-soft">{g.consumed} JH</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 w-20 bg-m-border-soft rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${consumedRate > 100 ? "bg-rose-500" : "bg-m-blue"}`} style={{ width: `${Math.min(consumedRate, 100)}%` }} />
                          </div>
                          <span className="font-mono-data text-xs font-semibold text-m-ink-soft">{consumedRate}%</span>
                        </div>
                      </td>
                    </tr>
                    {isOpen && g.rows.map((a) => (
                      <tr key={a.allocation_id} className="border-b border-m-border-soft bg-m-bg/60" data-testid={`resource-alloc-month-${a.allocation_id}`}>
                        <td />
                        <td className="px-4 py-2 pl-10 text-xs text-m-muted">Détail mensuel</td>
                        <td className="px-4 py-2 text-xs text-m-ink-soft capitalize">{fmtMonth(a.period_month)}</td>
                        <td className="px-4 py-2 font-mono-data text-xs text-m-muted">{a.allocation_rate != null ? `${a.allocation_rate}%` : "—"}</td>
                        <td className="px-4 py-2 font-mono-data text-xs text-m-ink">{a.jh_allocated} JH</td>
                        <td className="px-4 py-2 font-mono-data text-xs text-m-ink-soft">{a.jh_consumed ?? 0} JH</td>
                        <td />
                      </tr>
                    ))}
                  </React.Fragment>
