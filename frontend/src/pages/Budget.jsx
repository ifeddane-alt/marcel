...
function KpiCard({ label, value, sub, color = "blue" }) {
  ...
  return (
    <div className={`rounded-lg border border-zinc-200 border-l-4 p-4 ${colors[color]}`}>
      ...
      <p className="text-2xl font-bold text-zinc-800 mt-1">{fmtK(value)}</p>
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative bg-white w-full max-w-xl h-full shadow-2xl overflow-y-auto flex flex-col">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-start justify-between z-10">
          <div>
            <h2 className="font-bold text-zinc-800 text-base leading-tight">{project.name}</h2>
            <p className="text-xs text-zinc-400 mt-0.5">{project.program_name}</p>
          </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={l} className={`rounded-lg p-3 ${c}`}>
                      ...
                      <p className="text-lg font-bold text-zinc-800">{fmtK(v)}</p>
                    </div>
                  ))}
                </div>
...
export default function BudgetPage() {
  ...
  const thClass = "px-3 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wider text-zinc-500 cursor-pointer select-none hover:text-zinc-800 whitespace-nowrap";
  const tdClass = "px-3 py-2.5 text-sm text-zinc-700 whitespace-nowrap";
  ...
  const ProjectRow = ({ p, rowClass = "" }) => (
    ...
    >
      ...
      <td className={`${tdClass} text-center`}>
        <span className="text-xs text-zinc-500">{p.nb_revisions}</span>
      </td>
    </tr>
  );
  ...
  const TotalRow = ({ projects }) => {
    ...
    return (
      <tr className="bg-zinc-100 font-semibold border-t-2 border-zinc-300">
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6 min-h-screen bg-zinc-50">
      {/* En-tête */}
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-zinc-800" data-testid="budget-page-title">Budget Portefeuille</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => budgetAPI.exportExcel().then((r) => {
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-zinc-200 rounded-lg bg-white hover:bg-zinc-50 text-zinc-600">
        ...
        <>
          ...
          {envelope?.capex_envelope && (
            <div className="bg-white rounded-xl border border-zinc-200 p-5">
              <h3 className="text-sm font-semibold text-zinc-700 mb-3">Consommation vs Enveloppe 2026</h3>
          ...
          {/* Filtres */}
          <div className="flex flex-wrap gap-3 items-center bg-white rounded-xl border border-zinc-200 p-4">
            <select value={filterProgram} onChange={(e) => setFilterProgram(e.target.value)}
              data-testid="filter-program"
              className="border rounded-lg px-3 py-1.5 text-sm text-zinc-600 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white">
                ...
                <div key={pg.program_id || pg.program_name}
                  ...
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-zinc-800">{pg.program_name}</p>
                        <p className="text-xs text-zinc-400">{pg.nb_projects} projets</p>
                      </div>
                    </div>
                    <div className="flex gap-6 text-right text-sm">
                      <div>
                        <p className="text-xs text-zinc-400">Prévu</p>
                        <p className="font-medium">{fmtK(pg.capex_total + pg.opex_total)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-zinc-400">EAC</p>
                        <p className="font-medium">{fmtK(pg.eac_total)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-zinc-400">Écart</p>
