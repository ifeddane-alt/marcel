...
// ─── Config ────────────────────────────────────────────────────────────────
const URGENCY_CFG = {
  low:      { label: "Faible",   cls: "bg-zinc-100 text-zinc-600 border-zinc-200" },
...
const STATUS_COLS = [
  { key: "nouvelle",  label: "Nouvelle",  icon: Clock,         color: "border-t-slate-400",    hdr: "bg-zinc-50",   dot: "bg-zinc-400" },
...
function DemandCard({ demand, onClick, onDragStart }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-xl border border-zinc-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer p-4 select-none group"
    >
      {/* Urgency + score */}
      ...
      <div className="flex items-center gap-1.5 text-xs text-zinc-500 mb-2">
        ...
        {demand.requester_department && (
          <span className="text-zinc-400 truncate">• {demand.requester_department}</span>
        )}
      </div>
...
function KanbanColumn({ col, demands, onCardClick, onDragStart, onDrop, canWrite }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`${col.hdr} px-4 py-3 flex items-center justify-between border-b border-zinc-200`}>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${col.dot}`} />
          <span className="text-xs font-bold text-zinc-700">{col.label}</span>
        </div>
        <span className="text-xs font-bold text-zinc-500 bg-white rounded-full w-6 h-6 flex items-center justify-center border border-zinc-200">
          {demands.length}
        </span>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-zinc-200 px-6 py-4 flex-shrink-0">
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          {[
            { label: "Total", value: kpis.total, color: "text-zinc-700", bg: "bg-zinc-50 border-zinc-200", testId: "kpi-total" },
            { label: "Nouvelles", value: kpis.nouvelles, color: "text-zinc-600", bg: "bg-zinc-50 border-zinc-200", testId: "kpi-nouvelles" },
            ...
            <div key={k.testId} data-testid={k.testId} className={`rounded-xl border p-3 flex items-center gap-3 ${k.bg}`}>
              ...
              <div className="text-xs text-zinc-500 font-medium">{k.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {(filterStatus || filterUrgency || search) && (
            <button
              ...
              className="text-xs text-zinc-500 hover:text-rose-600 transition-colors"
            >
              Effacer filtres
            </button>
          )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide w-2/5">Demande</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Demandeur</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Urgence</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Statut</th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Budget</th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Score</th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Créée</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="font-medium text-zinc-800 line-clamp-1">{d.title}</div>
                        {d.requester_department && (
                          <div className="text-xs text-zinc-400 mt-0.5">{d.requester_department}</div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-zinc-600">{d.requester}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] border ${urg.cls}`}>
                      ...
                      <td className="px-4 py-3 text-right text-zinc-600 tabular-nums">{fmt(d.estimated_budget)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-none sm:rounded-2xl shadow-2xl w-full max-h-screen sm:max-h-[90vh] overflow-y-auto sm:max-w-sm p-6 space-y-4">
            ...
            {dndDialog.action === "prioritize" ? (
              <div>
                <label className="block text-xs font-semibold text-zinc-600 mb-1">Score (0–100)</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-600 mb-1">Motif de refus</label>
                <textarea
