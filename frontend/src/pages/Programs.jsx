  ...
  return (
    <div className="p-4 md:p-6 lg:p-8" data-testid="programs-page">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl sm:text-3xl font-bold text-zinc-950 tracking-tight">Programmes</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: "Budget total consolidé", value: formatEuro(programs.reduce((s, p) => s + (p.budget_total || 0), 0)), icon: TrendingUp, color: "text-blue-600" },
          { label: "Projets actifs", value: totalProjects, icon: FolderKanban, color: "text-zinc-700" },
          ...
          <div key={label} className="bg-white border border-zinc-200 rounded-lg shadow-sm p-4 flex items-center gap-4">
      ...
      <div className="space-y-4">
        {programs.map((prog) => {
          ...
          return (
            <div key={prog.program_id} className="bg-white border border-zinc-200 rounded-lg shadow-sm hover:shadow-md transition-shadow" data-testid={`program-card-${prog.program_id}`}>
              <div className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-xs text-zinc-400">{prog.project_count || 0} projet{(prog.project_count || 0) > 1 ? "s" : ""}</span>
                    </div>
                    <h2 className="font-heading text-lg font-bold text-zinc-950 leading-snug">{prog.name}</h2>
                    {prog.description && <p className="text-xs text-zinc-500 mt-1 line-clamp-2">{prog.description}</p>}
                    <div className="flex items-center gap-4 mt-2 text-xs text-zinc-400">
                      {prog.owner && <span>Owner : <span className="font-medium text-zinc-600">{prog.owner}</span></span>}
                      {prog.start_date && <span>{prog.start_date} → {prog.end_date || "—"}</span>}
                    </div>
                  </div>
                  <div className="flex-shrink-0 text-right space-y-1 min-w-[160px]">
                    <div className="text-xs text-zinc-400">Budget total</div>
                    <div className="font-mono-data text-base font-bold text-zinc-950">{(prog.budget_total_keur || 0).toLocaleString("fr-FR")} K€</div>
                    <div className="text-xs text-zinc-500">consommé : <span className={`font-semibold ${consumedPct > 90 ? "text-rose-600" : "text-zinc-700"}`}>{(prog.budget_consumed_keur || 0).toLocaleString("fr-FR")} K€</span></div>
                  </div>
                </div>

                <div className="mt-4">
                  <div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${consumedPct > 90 ? "bg-rose-500" : "bg-blue-600"}`} style={{ width: `${consumedPct}%` }} />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-3">
                  {[{ key: "green", color: "bg-emerald-500", label: "vert" }, { key: "orange", color: "bg-amber-500", label: "orange" }, { key: "red", color: "bg-rose-500", label: "rouge" }].map(({ key, color, label }) => (
                    <span key={key} className="flex items-center gap-1 text-[11px] text-zinc-500">
                      ...
                      <span className="font-semibold text-zinc-700">{(prog.rag_counts || {})[key] || 0}</span> {label}
                    </span>
                  ))}
        ...
        {programs.length === 0 && (
          <div className="bg-white border border-zinc-200 rounded-lg p-12 text-center text-zinc-400 text-sm">Aucun programme défini.</div>
        )}
      </div>
