...
## 2026-06 (fork) — Incident sélection indicateurs + onglets Dashboard/Indicateurs (Preview, NON déployé)
- INCIDENT : le testing agent (iteration_75) avait activé le preset P1 sur le dashboard d'admin@altair.fr → 11 indicateurs poussaient les widgets personnalisés hors écran → utilisateur furieux (« personnalisation supprimée »). Widgets/layouts JAMAIS perdus (vérifiés en base). Réparé : sélection remise à vide, section rendue repliable (localStorage indicators-collapsed-{scope}).
- LEÇON : toute activation de preset par un test sur un compte partagé doit être remise à l'état initial après test.
- Sur demande utilisateur : Dashboard scindé en 2 onglets « Tableau de bord » (grille widgets + Personnaliser) / « Mes indicateurs » (IndicatorsPanel) — persistance localStorage dashboard-view-tab, testids dashboard-view-tab-{dashboard|indicateurs}. Personnaliser masqué sur l'onglet indicateurs.
- Exports consolident les DEUX onglets : PDF COMEX (section « Mes indicateurs (sélection personnelle) » via catalog compute_values — testé avec/sans sélection), mode Présentation (slide « Mes indicateurs » si sélection non vide), Rapport IA (indicateurs injectés dans le contexte LLM via le router — le job hebdo reste sans).
- Testé : curl PDF (section présente avec sélection pmo, absente sans), bascule onglets UI + largeur grille OK au retour (WidthProvider remonté). Sélection du compte de test pmo remise à vide après test.

## 2026-06 (fork) — Onglets portefeuille + saisie manuelle indicateurs + RAG auto (DÉPLOYÉ PROD, commit 2de7008, bundle main.63390ccf.js)
- ✅ /portfolio : onglets « Portefeuille » / « Indicateurs » (pfTab, localStorage portfolio-view-tab, testids portfolio-view-tab-*) — ATTENTION : Portfolio.jsx avait déjà `view` (tuiles/liste) et `switchView` → renommés pfTab/switchPfTab ; hooks à déclarer AVANT le return conditionnel loading. ProgramDetail avait déjà son onglet indicateurs.
- ✅ Saisie manuelle indicateurs : PUT /api/indicator-catalog/manual/{scope}/{indicator_id} {value, unit, direction higher|lower, green, orange, context_id} — value:null efface ; collection indicator_manual_values ; compute_values intègre manual_map (display formaté + unité, status manual, rag par seuils, detail « Saisi le … par … », manual{} pour préremplissage, editable=fn is None). Front : crayon indicator-manual-edit-{id} → ManualValueModal (valeur/unité/direction/seuils) + pastille RAG indicator-rag-{id}. RBAC : scopes non-dashboard exigent has_perm(projects.edit) — require_write était trop permissif (READ_ONLY passait, faille signalée par iteration_76 ; require_write reste utilisé ailleurs, ex set_selection — dette connue).
- ✅ RAG auto projets : _compute_rag + _apply_auto_rag dans projects/service.py (appelés par list_projects et get_project, bulk_write si changement, projets cloture/archive/annule exclus). Règles : EAC vs budget (+10 % rouge/+5 % orange), glissement baseline→forecast (+90 j rouge/+30 j orange), jalons en retard (≥3 rouge/≥1 orange, date_forecast||date_baseline < today, status ∉ achieved/done), risques critiques ouverts (criticité ≥15, statuts identifié/en cours ; ≥2 rouge/1 orange). rag_reasons persistées + tooltips (RAGBadge prop reasons, tuiles title). Champ RAG RETIRÉ des formulaires (rag-auto-hint), payload sans status_rag (update_project ignore les None). Démo Altair : 5 red / 5 orange / 0 green (normal, horloge août 2026 → jalons passés).
- ✅ Fix GOU-04 : counts utilisait « amber » alors que le RAG est « orange » → 5 projets orange non comptés ; corrigé (normalise amber→orange).
- Testé iteration_76 : backend 16/18 → 2 HIGH corrigés et re-vérifiés par curl (viewer 403, GOU-04 « 0 V · 5 O · 5 R »), frontend 100 %. Suite : backend/tests/test_iter76_manual_rag.py. Le testing agent a nettoyé la sélection admin polluée par la suite legacy test_indicator_catalog.py (à corriger un jour : snapshot/restore).
- PROD : commit 2de7008, bundle main.63390ccf.js (tous marqueurs), conteneurs healthy, manual 403 sans auth, disque 60 %. Pendant le build le VPS sature (load ~35, SSH/site down ~5 min) — pattern connu, attendre puis vérifier uptime_seconds du health pour confirmer le redéploiement réel.
- Dette notée (iteration_76 minors) : status « manual » ambigu (saisi vs non calculable) ; _apply_auto_rag à chaque GET (2 aggregations + bulk) — OK à cette volumétrie ; throttling login 429 casse les suites pytest legacy exécutées en rafale.

## 2026-06 (fork) — Fix Personnaliser + Budget cible RAF/MVP (DÉPLOYÉS PROD, commits 8e633cf et 34e78ab)
- ✅ Fix : le bouton « Personnaliser » du Dashboard disparaissait sur l'onglet « Mes indicateurs » (et l'onglet est persisté) → l'utilisateur a cru la personnalisation supprimée (2e incident du même type !). RÈGLE : ne JAMAIS masquer un point d'entrée de fonctionnalité selon l'onglet actif — le bouton reste visible et bascule sur l'onglet Tableau de bord en mode édition. Prod : 8e633cf, bundle main.24f96ee4.js.
- ✅ Budget cible (BudgetTargetTab + forecast/service.get_levers/apply_cuts) : leviers projet désormais valorisés sur le RESTE À FAIRE réel (Σ tâches non terminées : jh_restants_estimes sinon jh_planned−jh_consumed, × TJM ressource) au lieu de budget_total−budget_consumed. Choix PAR PROJET : « Arrêt complet » (type pause, RAF total) ou « Réduire au MVP » (type reduce_mvp : sort du scope les tâches non-« sec », préserve le noyau MVP=scope sec, projet reste actif). Levier expose value_full/value_mvp/jh_mvp_preserved/value_mvp_preserved. UI : toggle pills par levier (lever-mode-full/mvp-{id}), label dynamique, mention « préserve le MVP (scope sec) : X JH · Y € ».
- PIÈGES appris : scope_status en base est hétérogène ('SEC'/'sec'/'etendu'/'out') → comparer en lower() / regex i ; statuts tâches terminées = done/termine/completed (le filtre historique ratait completed) ; TaskCreate n'a PAS scope_status (seulement TaskUpdate) → créer puis PUT pour poser le scope.
- Testé E2E : levier full 21 330 €/mvp 14 220 € (10 JH sec préservés), apply reduce_mvp → 1 seule tâche out, noyau sec conservé, projet NON pausé ; démo Phoenix : complet 2,66 M€ vs MVP 402 k€ (préserve 3 570 JH). Données de test supprimées. Prod : 34e78ab, bundle main.634b9421.js (markers lever-mode-mvp/reduce_mvp), levers 403 sans auth, disque 61 %.

## 2026-06 (fork) — Budget cible : correction perçue « erreur de calcul » DORA (DÉPLOYÉ PROD, commit a55806d, bundle main.177b3082.js)
- Signalement utilisateur : DORA « 894 k de MVP » → réduire au MVP devrait donner 0 de cut, or la barre ajoutait ~894 k. Investigation : DORA n'a QUE 55 JH marqués sec, 1180 JH SANS qualification de scope → comptés hors MVP. DÉCISION UTILISATEUR EXPLICITE : les tâches non qualifiées RESTENT dans les coupes (« il faut que les chefs de projet soient exigeants ») — le calcul reste, c'est la transparence qui manquait.
- ✅ Affichage : bouton « Réduire au MVP · économise X € », détail « conserve le MVP (scope sec) : X JH · Y € — coupe Z JH · ⚠ dont W JH non qualifiés (V €) — qualifiez le scope des tâches » (champs jh_unqualified/value_unqualified sur le levier).
- ✅ VRAI bug corrigé au passage : les tâches déjà scope « out » étaient comptées dans le RAF des leviers (DORA 894 k→691 k, Phoenix 4235→3587 JH) ; apply reduce_mvp exclut aussi les out du update (^(sec|out)$).
- Nettoyage : 3 entrées budget_cuts créées par mes tests E2E supprimées de la base Preview (l'historique des coupes persiste même quand le projet de test est supprimé — penser à purger budget_cuts après les tests apply-cuts).
- Prod vérifiée : commit a55806d, bundle marker « qualifiez le scope », health ok, disque 62 %.
...
## 2026-06 (fork) — Qualification scope en masse + Restauration coupes + Taux qualification portefeuille (DÉPLOYÉ PROD, commit 96819a2, bundle main.1bb3ae7c.js)
- ✅ Qualification rapide : cases à cocher dans la liste des tâches (ProjectDetail, vue Liste, canWrite) + barre d'actions bulk « Noyau MVP (sec) / Étendu / Hors scope » (bulk-scope-bar, bulk-scope-{sec,etendu,out}-btn). POST /api/tasks/bulk-scope {task_ids, scope_status} — RBAC permission_required("projects.edit") (require_write est LAXISTE : le viewer a des perms non-view comme export.ppt → _write passe ; piège à retenir pour tout nouvel endpoint tâches). Badge scope (SEC/ÉTENDU/OUT) ou « À QUALIFIER » (ambre, tâches non terminées) dans la colonne Nom (task-scope-badge-{id}).
- ✅ Annulation de coupe : apply_cuts mémorise désormais details[].restore (task/reduce_mvp : [{task_id, prev_scope}] ; pause : project_status). POST /api/forecast/cuts/{cut_id}/restore (budget.edit) : remet le scope d'origine (prev_scope falsy enregistré → $unset = redevient non qualifiée ; coupe legacy sans mémoire → etendu ; pause → statut d'origine sinon actif), marque restored/restored_by/restored_at, 409 si déjà restaurée. UI : bouton « Restaurer » (cut-restore-btn-{id}) + badge « Restaurée le … » (cut-restored-badge-{id}) + montant barré.
- ✅ Indicateur qualification : GET /api/projects/scope-qualification (par projet actif : JH restants qualifiés sec/etendu/out vs non qualifiés, pct_qualified, tri croissant, respect view_own). Portefeuille : bandeau indigo repliable (portfolio-qualification-alert) « N projets avec un scope non qualifié », lignes cliquables → fiche ?tab=taches avec % coloré (<50 rouge, <80 ambre) + mini barre. Cohérence vérifiée : DORA 19 % / 1180 JH non qualifiés = mêmes chiffres que le Budget cible.
- Testé : script E2E 22/22 PASS (bulk sec/etendu, 422 invalide/vide, cut task→restore etendu, reduce_mvp→restore (sec conservé, non-qualifiée redevient non qualifiée), pause→restore actif, double restore 409, viewer 403 après fix RBAC) + 3 screenshots Preview (bandeau 6 projets, barre bulk, bouton Restaurer→toast+badge). Données de test purgées (projets ZZ-TEST supprimés, budget_cuts=0).
- ⚠ INCIDENT DEPLOY : le token GitHub du remote origin est EXPIRÉ/RÉVOQUÉ (push → « Invalid username or token »). Déploiement fait par push SSH direct vers le repo VPS (/opt/marcel, receive.denyCurrentBranch=updateInstead) puis update.sh. GitHub (public, HEAD 0fb8377) est maintenant EN RETARD sur prod/pod (96819a2) → l'utilisateur doit re-synchroniser via « Save to Github » ou fournir un token frais. update.sh reste sûr : son git pull origin main no-op (local en avance).
- Prod vérifiée : health 200 uptime reset, 3 routes 403 sans auth, bundle main.1bb3ae7c.js avec les 5 marqueurs, permissions synchronisées 2 tenants, prune 4,02 Go → disque 43 % (9,7 Go libres).

## 2026-06 (fork) — Scénarios de coupe versionnés (DÉPLOYÉ PROD, commits 4e1836a + 56fd038, bundle main.1e8aef98.js)
- ✅ Collection cut_scenarios : {scenario_id, lineage_id, version, name, target, items[{type,id,mode,label,value_saved}], total_saved, status draft/applied, applied_cut_id}. GET/POST /api/forecast/scenarios (POST avec lineage_id → V(n+1), sans → nouvelle lignée V1 ; 422 nom/items vides, 404 lignée inconnue), DELETE /{id} (204/404). RBAC budget.view / budget.edit.
- ✅ apply-cuts accepte scenario_id → scénario marqué applied + applied_cut_id ; restore d'une coupe liée → scénario redevient draft.
- ✅ UI BudgetTargetTab : toolbar scénario (scenario-save-btn « Enregistrer le scénario »/« Enregistrer en V{n+1} », input nom scenario-name-input, scenario-new-btn from scratch, badge scenario-loaded-badge), panneau scenarios-panel (V badge, date/auteur/leviers/cible, total, badge Appliqué, Charger/Supprimer). Chargement : re-sélection des leviers actuels avec montants recalculés à date, warning si levier disparu.
- Testé : 19/19 E2E backend + cycle UI complet Preview (V1 → nouveau → charger → V2 même lignée V1 intacte → suppression). Données test purgées (cuts=0, scénarios=0).
- 🔥 INCIDENT PROD 26/08 : PANNE RÉSEAU ens2 (PAS de gel machine, PAS d'OOM). Preuves syslog : 13:35 UTC « Failed to configure NDisc route: Connection timed out » en boucle (104 occurrences), 16:48 « The interface entered the failed state frequently, refusing to reconfigure it automatically » → serveur vivant mais injoignable jusqu'au reboot utilisateur (21:11 UTC). Prémices le 25/08 (3 erreurs). Aucun lien avec le push git (coïncidence temporelle). Le journalctl du boot gelé est perdu — utiliser /var/log/syslog (format ISO 2026-08-26T…).
- ⚠ Le reboot Scaleway RÉINITIALISE authorized_keys → clé pod perdue. Procédure de récupération : ajouter la clé publique du pod (~/.ssh/id_ed25519.pub) dans console.scaleway.com/project/ssh-keys puis Reboot de l'instance (cloud-init réinstalle les clés). Fait par l'utilisateur le 27/08.
- ✅ Protections installées sur le VPS : net-watchdog (systemd timer 2 min : ping 8.8.8.8, sinon networkctl reconfigure ens2 puis restart systemd-networkd — aurait auto-réparé l'incident sans reboot) ; swapfile 2 Go persistant (fstab + vm.swappiness=15 via /etc/sysctl.d/99-swap.conf ; la machine a 3,9 Go RAM sans swap) ; update.sh reload nginx après rebuild (le backend change d'IP conteneur → 502 sinon ; incident observé, corrigé par docker exec marcel-nginx-http-1 nginx -s reload).
- Prod vérifiée : health OK, scenarios 403 sans auth / 200+[] admin, bundle main.1e8aef98.js avec 4 marqueurs, prune 1,9 Go → disque 55 % (7,7 Go libres).
- ⚠ Token GitHub toujours expiré : déploiements via push SSH direct (ssh://root@VPS/opt/marcel, receive.denyCurrentBranch=updateInstead). GitHub en retard sur prod — re-sync « Save to Github » à faire par l'utilisateur.

## 2026-06 (fork) — Jeu de données « MARCEL full opérationnel » Banque/Assurance (DÉPLOYÉ PROD, commits 70a2463 + 7aa2bad)
- ✅ Script /app/scripts/seed_bank_full.py (reproductible, random.seed(42)) : purge les données métier du tenant Altair (PRÉSERVE users/profils/tenant/référentiels/préférences/indicator_selections) puis génère : 13 programmes, 350 projets (répartition inégale réaliste, ~195 actifs / 46 prépa / 23 pause / 92 clôturés), ~4 300 tâches (JH cohérents, scope qualifié à 3 niveaux full/partial/bad → ~83 projets sous 100 %), ~1 950 jalons (retards ciblés), ~700 risques, 220 dépendances inter-projets, 100 applications bancaires (TIME model, composants obsolètes) + 160 interfaces, 260 ressources / 28 équipes / ~4 800 allocations (cap 20 JH/mois/ressource), timesheets/congés, Run (53 activités, 260 incidents 12 mois, 61 MEP, 90 vulnérabilités, 24 exigences DORA/NIS2/RGPD/ISO/LCB-FT/DSP3, revues sécu), Architecture (12 standards, 6 dérogations, 40 revues, radar 16, dette 70), Gouvernance (16 comités, 88 gates, 130 décisions, 45 demandes), SAFe (3 ARTs, 9 PIs, 6 sprints, 30 capabilities WSJF, 2 PB sessions), OKRs 12, 8 objectifs stratégiques, budget (enveloppes 2026/2027 cohérentes avec Σ projets, 13 enveloppes programmes, 8 transferts, 40 reforecasts), 8 snapshots portefeuille, index Mongo de perf.
- Cohérence : budgets/JH projets = agrégats des tâches (12 projets volontairement divergents pour l'alerte cohérence → 9 détectés) ; RAG auto calculé : actifs 97 verts / 66 orange / 32 rouges ; qualification scope cohérente entre portefeuille et budget cible.
