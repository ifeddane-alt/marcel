...
@api_router.post("/export/copil")
async def export_copil(data: ExportCopilRequest, current_user: TokenPayload = Depends(get_current_user)):
    if not data.project_ids:
        raise HTTPException(status_code=422, detail="Au moins un projet requis")

    # Fetch all required data
    projects = await db.projects.find(
        {"project_id": {"$in": data.project_ids}, "tenant_id": current_user.tenant_id}, {"_id": 0}
    ).to_list(None)
    if not projects:
        raise HTTPException(status_code=404, detail="Aucun projet trouvé pour les IDs fournis")

    # Resolve owner names
    resources = await db.resources.find({"tenant_id": current_user.tenant_id}, {"_id": 0, "resource_id": 1, "name": 1}).to_list(None)
    res_map = {r["resource_id"]: r["name"] for r in resources}
    for p in projects:
        p["owner_name"] = res_map.get(p.get("owner_id", ""), p.get("metadata", {}).get("sponsor", "—") or "—")

    # Keep ordering consistent with input order
    ...
    decisions_query: dict = {"project_id": {"$in": data.project_ids}, "tenant_id": current_user.tenant_id}
    ...
    safe_name = "".join(c if c.isalnum() else "_" for c in data.instance_name)[:40]
    filename = f"COPIL_{safe_name}_{data.instance_date}.pptx"
    return Response(
        content=buf.read(),
