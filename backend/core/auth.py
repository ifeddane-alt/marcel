...
def create_token(payload: dict, *, expires_delta: timedelta | None = None) -> str:
    ttl = expires_delta or timedelta(hours=ACCESS_TOKEN_HOURS)
    data = {**payload, "exp": datetime.now(timezone.utc) + ttl}
...
async def _verify_revocation(tp: TokenPayload) -> None:
    ...
    try:
        ...
        logger.error("perm_version check indisponible: %s", e)
        raise HTTPException(status_code=503, detail="Service d’authentification temporairement indisponible")
    if u is None:
