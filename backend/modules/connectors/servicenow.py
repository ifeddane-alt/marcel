...
async def test_connection(base_url: str, auth_type: str, credentials: dict) -> dict:
    ...
    try:
        ...
        async with hardened_async_client(timeout=8, verify=connector_tls_verify(), auth=(user, pw)) as client:
...
async def _real_sync(config: dict, direction: str) -> dict:
    ...
    async with hardened_async_client(timeout=30, verify=connector_tls_verify(config), auth=(user, pw)) as client:
        resp = await client.get(url, headers={"Accept": "application/json"})
        if resp.status_code != 200:
